...
def main():
    ...
    for iPr in tqdm(range(len(textListALL))):
        ...
        for Clase in Clases:
            if voteBox[Clase[0]]==None:
                ...
                voteBox[Clase[0]]+=Clase[1]
        # print(voteBox)
        # print(">>>>",[kv for kv in voteBox.items() if kv[1] == max(voteBox.values())])#voting results an score
        # print(">>>>",[kv[0] for kv in voteBox.items() if kv[1] == max(voteBox.values())])
    ...
    with open('LocalLLM/OneLLMEmbVoting/answer_template.json', 'w') as file:
