...
# Annex: Approach 4 - Word2Vec-based Vector Comparison

This annex describes a fourth approach that utilizes **Word2Vec** embeddings for categorizing products. In this method, pre-trained Word2Vec models are used to generate word-level embeddings, which are then fine-tuned on the provided dataset. The approach involves several key steps:

1. **Downloading and Loading Pre-trained Models:**  
   The `downloadModels.py` script downloads multiple pre-trained models (e.g., Google News vectors, GloVe, FastText, and ConceptNet Numberbatch). These models offer a solid baseline for word representations.

2. **Vector Comparison and Preprocessing:**  
   The `RunVectorComparation.py` script demonstrates how to preprocess text from the categories and products CSV files. It cleans, tokenizes, and chunks the data—combining relevant columns (such as merging `metaTitle` with `vendorCategory`)—and then converts these text segments into vectors using Word2Vec.

3. **Fine-tuning the Word2Vec Model:**  
   The `RunWord2VecFN.py` script fine-tunes a Word2Vec model on the custom dataset. It builds a vocabulary from the tokenized sentences, incorporates pre-trained embeddings, and further trains the model on the provided data to better capture domain-specific language.

**Advantages of the Word2Vec Approach:**
- **Rich Word-Level Representations:**  
  Word2Vec effectively captures semantic and syntactic relationships between words, which is beneficial for processing nuanced product descriptions.
- **Fine-Tuning Capability:**  
  By fine-tuning on your specific dataset, the model adapts to domain-specific terminology, enhancing its categorization performance.
- **Complementarity:**  
  This method offers an alternative perspective to the other approaches and can be integrated into an ensemble system for improved accuracy.

While the Word2Vec approach may require additional computational resources for fine-tuning and vector comparisons, it provides a valuable alternative for scenarios where a fine-grained, word-level understanding of text is critical.

---

**Files related to the Word2Vec Approach:**
- `downloadModels.py`
- `RunVectorComparation.py`
- `RunWord2VecFN.py`
- `Wor2VecFN.py`
